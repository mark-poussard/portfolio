...
import './DoubleCaret.scss';

interface IDoubleCaretProps{
    left ?: boolean;
    right ?: boolean;
}

const DoubleCaret : React.FC<IDoubleCaretProps> = props => {
    const directionClassName = (props.left === true)?"left":"right";
    return (
        <InlineSvg viewBox="0 0 448 512"
            className={`double-caret ${directionClassName}`}>
            {props.left === true &&
